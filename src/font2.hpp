...
Font load_font(String filepath, f32 size, Allocator *allocator)
{
  ...
  font.space_width = font.characters[' '].advance;
...
//     Rect4f shape_rect = {x + c.shape.x, y + baseline + (v_scale * c.shape.y),
...
//   for (i32 i = 0; i < text.size; i++) {
//     Character c     = font.characters[text.data[i]];
//     Rect4f shape_rect = {x + c.shape.x, y + baseline + c.shape.y,
//     c.shape.width,
//                        c.shape.height};
//     x += c.advance;
...
//                         Rect4f sub_target, f32 border, f32 scale,
