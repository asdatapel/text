...
struct Args {
    ...
    metal::texture2d<float> textures[128];
};
...
f32 compute_coverage(ConicCurvePrimitive curve, Vec2f origin, f32 rotation, f32 pixel_width, f32 t)
{
...
fragment float4 fragment_shader(VertexOut in [[stage_in]], constant Args *args) {
    ...
    if (in.primitive_type == ROUNDED_RECT) {
      ...
      out = args->textures[in.texture_idx].sample(sampler, in.uv);
      out.a *= clip(in.position, in.clip_rect_bounds);
    } else if (in.primitive_type == BITMAP_GLYPH) {
      out = in.color;
      out.a = args->textures[in.texture_idx].sample(sampler, in.uv).r;
      out.a *= clip(in.position, in.clip_rect_bounds);
      ...
      for (u32 curve_i = 0; curve_i < glyph.curve_count; curve_i++) {
        ...
        for (u32 s = 0; s < n_samples; s++) {
          ...
          f32 width = metal::length(metal::fwidth(in.uv) * Vec2f(metal::cos(angle), metal::sin(angle))) * 1.5;
          coverage += compute_coverage(curve, in.uv, angle, width, primitives->canvas_size.z) /
                      n_samples;
        }
      }
